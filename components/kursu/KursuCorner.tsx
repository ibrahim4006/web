...
const KursuCorner = ({ number, clickedNumber, setClickedNumber }) => {
  ...
  const corners = [
    ...
    {
      line: "-scale-x-100 -scale-y-100 bottom-0",
  ...
  return (
    <section key={number} className="relative">
      <div className={`right-lines w-[546.34px] ${corners[number].line}`}>
        <svg
          viewBox="0 0 504 444"
          fill="none"
      ...
      <div
        className={`kursu-outer-box inverse-hover w-[258.5px] absolute ${corners[number].box}`}
        onClick={() => {
          setClicked(!clicked);
