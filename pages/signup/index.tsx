...
const Page: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const cardTimeouts = [500, 650, 800, 950, 1100, 1250, 1300];
  ...
  function renderCard(index: number): JSX.Element {
    if (index === 6) {
      return (
        <div className="relative h-screen w-screen center z-50 flex">
          <div className=" relative w-[435px] h-[600px] flex flex-col items-center bg-[#f7f6f1] text-white text-opacity-80 animate_content_closing">
            ...
            <div className="relative h-14 w-[80%] flex items-center my-2 group overflow-visible">
              ...
              <form className="z-20 w-full h-14 flex center">
                <input
                  type="email"
                  className="w-[88%] h-12 bg-transparent border-transparent focus:border-y focus:border-opacity-30 focus:border-[#0d0d0d] outline-0 text-[#0d0d0d] px-4 group-hover:px-6 duration-100 inverse-hover"
                  placeholder="e-mail"
                />
              </form>
            </div>
